...
impl SqliteRepo {
    pub async fn connect(cfg: &DbConfig) -> RepoResult<Self> {
        ...
        if let Some(path) = cfg.url.strip_prefix("sqlite://") {
            if path != ":memory:" {
        ...
        let pool = sqlx::SqlitePool::connect(&cfg.url).await?;
        // Pragmas for production-grade reliability
...
impl Repository for SqliteRepo {
    ...
    async fn run_migrations(&self) -> RepoResult<()> {
        // Migrations mirror src-tauri/src/db.rs behavior:
        // 1. themes.name UNIQUE migration (table already created with UNIQUE so no-op)
        // 2. jobs flexible-schema migration (CHECK constraints removed — only matters on legacy DBs)
        // 3. jobs idempotent ALTER TABLE ADD COLUMN loop for cols added later
        let cols: Vec<String> = sqlx::query("PRAGMA table_info(jobs)")
            .fetch_all(&self.pool)
            .await?
            .iter()
        ...
        for col in missing {
            if !cols.iter().any(|c| c == col) {
                let sql = format!("ALTER TABLE jobs ADD COLUMN {} TEXT", col);
                sqlx::query(&sql).execute(&self.pool).await.ok(); // ignore failures
            }
        }
    ...
    async fn list_inbox(&self) -> RepoResult<Vec<InboxJob>> {
        let rows = sqlx::query(
            "SELECT id, url, raw_description, status, created_at FROM inbox_jobs ORDER BY created_at DESC",
        )
        .fetch_all(&self.pool)
        .await?;
        rows.into_iter()
    ...
    async fn get_inbox(&self, id: &str) -> RepoResult<Option<InboxJob>> {
        let row = sqlx::query(
            "SELECT id, url, raw_description, status, created_at FROM inbox_jobs WHERE id = ?1",
        )
        .bind(id)
        ...
        if let Some(meta) = metadata {
            if let Some(obj) = meta.as_object() {
                for (k, v) in obj {
                    ...
                    // Whitelist of allowed metadata columns
                    if matches!(
                        k.as_str(),
                        "applied_date"
                            | "interview_date"
                        ...
                        let sql = format!("UPDATE jobs SET {} = ?1 WHERE id = ?2", k);
                        sqlx::query(&sql)
                            .bind(value)
                            .bind(id)
                            .execute(&mut *tx)
    ...
    async fn update_job_metadata(&self, id: &str, field: &str, value: &str) -> RepoResult<()> {
        ...
        let sql = format!("UPDATE jobs SET {} = ?1 WHERE id = ?2", field);
        sqlx::query(&sql)
            .bind(value)
            .bind(id)
            .execute(&self.pool)
    ...
    async fn list_resumes(&self) -> RepoResult<Vec<ResumeItem>> {
        let rows = sqlx::query(
            "SELECT id, name, category, created_at, updated_at FROM base_resumes ORDER BY updated_at DESC",
        )
        .fetch_all(&self.pool)
        .await?;
        rows.into_iter()
    ...
    async fn get_resume(&self, id: &str) -> RepoResult<Option<ResumeDetail>> {
        let row = sqlx::query(
            "SELECT id, name, category, latex_content, created_at, updated_at FROM base_resumes WHERE id = ?1",
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    async fn list_cover_letters(&self) -> RepoResult<Vec<CoverLetterItem>> {
        let rows = sqlx::query(
            "SELECT id, name, category, created_at, updated_at FROM base_cover_letters ORDER BY updated_at DESC",
        )
    ...
    async fn get_cover_letter(&self, id: &str) -> RepoResult<Option<CoverLetterDetail>> {
        let row = sqlx::query(
            "SELECT id, name, category, latex_content, created_at, updated_at FROM base_cover_letters WHERE id = ?1",
        )
    ...
    async fn list_downloads(&self) -> RepoResult<Vec<DownloadRecord>> {
        let rows = sqlx::query(
            "SELECT id, filename, download_type, job_id, content_id, created_at
    ...
    async fn save_custom_theme(&self, id: &str, name: &str, config: &str) -> RepoResult<()> {
        sqlx::query("INSERT INTO themes (id, name, config, is_builtin) VALUES (?1, ?2, ?3, 0)")
            .bind(id)
            .bind(name)
            .bind(config)
            .execute(&self.pool)
    ...
    async fn delete_theme(&self, id: &str) -> RepoResult<()> {
        sqlx::query("DELETE FROM themes WHERE id = ?1 AND is_builtin = 0")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    async fn upsert_theme(
        ...
        .bind(id).bind(name).bind(config).bind(is_builtin as i64)
        .execute(&self.pool).await?;
        Ok(())
    }
    ...
    async fn list_documents(&self) -> RepoResult<Vec<DocumentSummary>> {
        let rows = sqlx::query(
            "SELECT id, title, description, tags, starred, main_file, last_compiled_at, compile_status, created_at, updated_at
    ...
    async fn create_document(
        ...
        .bind(&id).bind(title).bind(description).bind(tags).bind(starred as i64)
        .execute(&self.pool).await?;
        Ok(id)
    }
    ...
    async fn list_document_files(&self, doc_id: &str) -> RepoResult<Vec<DocumentFileEntry>> {
        let rows = sqlx::query(
            "SELECT rel_path, size_bytes, updated_at FROM document_files WHERE doc_id = ?1 ORDER BY rel_path",
    ...
    async fn rename_document_file(
        ...
        .execute(&mut *tx)
        .await?;
        // Update descendants (paths that start with old_path + '/')
        ...
        // Update main_file if it points to this or a descendant
    ...
    async fn export_all(&self) -> RepoResult<FullBackup> {
        ...
        let cl_rows = sqlx::query("SELECT id, name, category, latex_content, created_at, updated_at FROM base_cover_letters")
            .fetch_all(&self.pool).await?;
        ...
        let df_rows = sqlx::query(
            "SELECT doc_id, rel_path, content, size_bytes, updated_at FROM document_files",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn import_all(&self, backup: &FullBackup) -> RepoResult<()> {
        ...
        sqlx::query("DELETE FROM base_cover_letters")
            .execute(&mut *tx)
            .await?;
        sqlx::query("DELETE FROM themes WHERE is_builtin = 0")
            .execute(&mut *tx)
            .await?;
...
fn map_job_row(r: sqlx::sqlite::SqliteRow) -> RepoResult<JobPayload> {
    Ok(JobPayload {
        id: r.try_get("id")?,
...
fn map_document_row(r: sqlx::sqlite::SqliteRow) -> RepoResult<DocumentSummary> {
    Ok(DocumentSummary {
        id: r.try_get("id")?,
