...
impl Repository for PgRepo {
    ...
    async fn export_all(&self) -> RepoResult<FullBackup> {
        // SECURITY: 'ai_api_key' is intentionally excluded from export. The
        // encrypted blob stays on this machine only.
    ...
    async fn import_all(&self, backup: &FullBackup) -> RepoResult<()> {
        ...
        for (k, v) in &backup.app_settings {
            // Don't restore ai_api_key from S3; the encrypted key never leaves this machine.
            if k == "ai_api_key" {
                continue;
            }
        ...
        if let Some(cs) = &backup.compiler_state {
            ...
            .bind(&cs.latex_content).execute(&mut *tx).await.ok();
        }
        for d in &backup.downloads {
            sqlx::query(
                "INSERT INTO downloads (id, filename, download_type, job_id, content_id, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
            )
        ...
        for j in &backup.inbox_jobs {
            sqlx::query(
                "INSERT INTO inbox_jobs (id, url, raw_description, status, created_at) VALUES ($1, $2, $3, $4, $5)",
            )
        ...
        for f in &backup.document_files {
            sqlx::query(
                "INSERT INTO document_files (doc_id, rel_path, content, size_bytes, updated_at) VALUES ($1, $2, $3, $4, $5)",
            )
            .bind(&f.doc_id).bind(&f.rel_path).bind(&f.content).bind(f.size_bytes as i64).bind(&f.updated_at)
            .execute(&mut *tx).await.ok();
        }
        tx.commit().await?;
        Ok(())
