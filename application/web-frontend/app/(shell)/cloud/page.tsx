      ...
      <header className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-[var(--line)] pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-[var(--muted)] mt-1">
            Encrypted snapshot sync to AWS S3, Cloudflare R2, MinIO, Wasabi, or
            B2 bucket.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            disabled={testing}
            className="flex items-center gap-1.5 h-10 px-3.5 rounded-lg bg-[var(--surface-soft)] border border-[var(--line)] text-xs font-bold text-[var(--ink)] hover:border-[var(--muted)]"
          >
            {testing ? (
          ...
          <button
            ...
            disabled={uploading}
            className="flex items-center gap-1.5 h-10 px-4 rounded-lg bg-[var(--accent)] text-white text-xs font-bold hover:opacity-90 disabled:opacity-50"
          >
            {uploading ? (
              <RotateCw className="h-4 w-4 animate-spin" />
            ) : (
